...
describe("MarketGeneration", function () {
    ...
    beforeEach(async function () {
        ...
        marketGeneration = await marketGenerationFactory.connect(owner).deploy(dev.address);
    ...
    it("active-only functions fail when not active", async function () {
        ...
        await expect(marketGeneration.connect(user1).contribute(user2.address, 2)).to.be.revertedWith("Distribution not active");
    })
    ...
    describe("activate() called", function () {
        ...
        describe("User 1/2/3 contributes 1/2/3 Target Tokens to round 1/2/3", async function () {
            beforeEach(async function () {
                ...
                await marketGeneration.connect(user1).contribute(owner.address, utils.parseEther("1"));
                

                await marketGeneration.connect(user1).contribute(user1.address, utils.parseEther("2"));
                

                await marketGeneration.connect(user3).contribute(user2.address, utils.parseEther("3"));
            })
            ...
            it("sends referral points to dev if no referral is specified", async function () {
                await marketGeneration.connect(owner).contribute(constants.AddressZero, utils.parseEther("10"));
            ...
            describe("Distribution contract changed", function () {
                ...
                it("claim() works", async function () {
                    ...
                    console.log('BEFORE CLAIM 2');
                    await marketGeneration.connect(user2).claim();
                    console.log('BEFORE CLAIM 3');
